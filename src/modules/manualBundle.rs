...
impl Bundler {
    ...
    ) -> Result<Vec<VersionedTransaction>, BundlerError> {
    ...
    pub async fn send_bundle(&self, bundle_transactions: Vec<VersionedTransaction>) -> Result<(), BundlerError> {
        self.send_bundle_to_jito(&bundle_transactions, &self.block_engine).await?;
        Ok(())
    }
    ...
    pub async fn execute_create_and_bundle(
        ...
        let bundle_transactions = self.prepare_bundle(total_buy_amount, jito_tip, dev_buy_amount).await?;
        self.send_bundle(bundle_transactions).await?;
        Ok(())
    }

    pub async fn cleanup_lut(&self, lookup_table_address: Pubkey) -> Result<(), BundlerError> {
...
pub async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Manual Bundle Launch");
    // You may want to load these from env or prompt
    ...
    // Prompt for buy amount, tip, etc.
    print!("\x1b[36mEnter total buy amount (SOL) --> \x1b[0m");
    io::stdout().flush()?;
    let mut total_buy_amount = String::new();
    io::stdin().read_line(&mut total_buy_amount)?;
    let total_buy_amount: f64 = total_buy_amount.trim().parse()?;

    print!("\x1b[36mEnter Jito tip (SOL) --> \x1b[0m");
    io::stdout().flush()?;
    let mut jito_tip = String::new();
    io::stdin().read_line(&mut jito_tip)?;
    let jito_tip: f64 = jito_tip.trim().parse()?;

    // Optionally prompt for dev buy amount
    let dev_buy_amount = None;

    // Phase 1: Prepare bundle (Create LUT + Create token + Prepare bundle transactions)
    println!("Preparing bundle (creating LUT and token)...");
    let bundle_transactions = bundler.prepare_bundle(total_buy_amount, jito_tip, dev_buy_amount).await?;
    println!("Bundle prepared successfully!");
    println!("Ready to send the bundle. Type 'y' and press Enter to send, or anything else to abort:");
    ...
    if confirm.trim().to_lowercase() == "y" {
        ...
        bundler.send_bundle(bundle_transactions).await?;
        println!("Bundle sent successfully!");
    } else {
        println!("Aborted. Bundle was not sent.");
    }
    Ok(())
}